...
export const AboutPage = () => {
  ...
  const [modifySelectedDataset, setModifySelectedDataset] = useState<string>(() => {
    // 如果当前数据集有记录，使用当前数据集，否则使用第一个有记录的数据集
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      const newBuffer = (inputBuffer + event.key).slice(-6); // 保留最后6个字符
      setInputBuffer(newBuffer);

      if (newBuffer === "modify") {
        setIsModifyEnabled(true);
        log.info("Data modification enabled", "AboutPage");
      }
    };
    ...
    if (dataset) {
      ...
      if (lessons.length > 0) {
        ...
        if (records && records.length > 0) {
          ...
          setModifySelectedRecord(latestRecordIndex); // 设置最新记录
        }
      }
    }
  ...
  useEffect(() => {
    if (availableRecords.length > 0) {
      ...
      setModifySelectedRecord(latestRecordIndex); // 自动选择最新记录
    }
  ...
  const handleModify = () => {
    ...
    // 根据结果处理
    if (success) {
      log.info("Record modified successfully", "AboutPage");
      setIsModifyDialogOpen(false);
    } else {
      log.error("Failed to modify record", "AboutPage");
    }
  };
